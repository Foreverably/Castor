...
@StaffOnly()
export default class MuetCommand extends BaseSlashCommand
{
        ...
        super(client, {
            ...
            construct: () =>
                new Builder<"SlashCommandBuilder">()
                    .setName("muet")
                    .setDescription("Muet someone!")
                    .addUserOption((option) =>
                        option
                            .setName("user")
                            .setDescription("The user to muet.")
                            .setRequired(true),
                    )
                    .addStringOption((option) =>
                        option
                            .setName("reason")
                            .setDescription("The reason for the muet."),
                    ),
        });
    }
    ...
    async execute(interaction: Interaction<"ChatInput">): Promise<void>
    {
        ...
        {
            await interaction.reply({
                content: "Please specify a user to muet!",
                flags: MessageFlags.Ephemeral,
            });
            return;
        }
        ...
        const embed = new EmbedBuilder()
            .setColor(0x38f075)
            .setDescription(
                `${Emoji.Check} <@${targetMember.id}> **was mueted** || ${reason}`,
            );
